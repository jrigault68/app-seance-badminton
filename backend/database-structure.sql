...
-- Table des utilisateurs (existante, à étendre si nécessaire)
...
CREATE TABLE IF NOT EXISTS categories_exercices (
...
CREATE TABLE IF NOT EXISTS types_exercices (
    id SERIAL PRIMARY KEY,
...
CREATE TABLE IF NOT EXISTS seances (
    ...
    description TEXT,
    niveau_id INTEGER REFERENCES niveaux_difficulte(id),
    type_seance VARCHAR(50), -- 'mobilité', 'renforcement', 'cardio', 'mixte', etc.
    categories JSONB DEFAULT '[]',
    objectifs JSONB DEFAULT '[]',
    duree_estimee INTEGER, -- en minutes
    calories_estimees INTEGER,
    materiel_requis JSONB DEFAULT '[]',
    structure JSONB NOT NULL, -- Structure complète de la séance
    notes TEXT,
    tags JSONB DEFAULT '[]',
    difficulte_ressentie DECIMAL(3,1), -- 1.0 à 10.0
    note_utilisateur DECIMAL(3,1), -- 1.0 à 5.0
...
-- Table des séances personnalisées (séances créées par les utilisateurs)
CREATE TABLE IF NOT EXISTS seances_personnalisees (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS sessions_entrainement (
    ...
    seance_personnalisee_id UUID REFERENCES seances_personnalisees(id),
...
CREATE INDEX IF NOT EXISTS idx_seances_type ON seances(type_seance);
CREATE INDEX IF NOT EXISTS idx_seances_auteur ON seances(auteur_id);
...
INSERT INTO categories_exercices (nom, description, couleur, icone, ordre_affichage) VALUES
...
INSERT INTO types_exercices (nom, description) VALUES
...
CREATE TRIGGER update_seances_personnalisees_updated_at BEFORE UPDATE ON seances_personnalisees FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
COMMENT ON TABLE categories_exercices IS 'Catégories d''exercices (échauffement, mobilité, etc.)';
...
COMMENT ON TABLE types_exercices IS 'Types d''exercices (temps, répétitions, etc.)';
COMMENT ON TABLE exercices IS 'Exercices disponibles dans l''application';
COMMENT ON TABLE seances IS 'Séances prédéfinies';
COMMENT ON TABLE seances_personnalisees IS 'Séances créées par les utilisateurs';
