  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center flex-col gap-4 bg-gradient-to-br from-red-950 via-black to-red-900 text-white px-4">
      ...
      <div className="max-w-xl w-full bg-white/10 rounded-2xl p-6 shadow-2xl space-y-4 text-center">
        <h2 className="text-2xl font-bold text-orange-300">Repos, prépare-toi pour la suite !</h2>
        ...
        <div className="flex justify-center gap-4 pt-4" style={{ gridGap: "75px", alignItems: "center" }}>
          <button onClick={() => {setPaused(!paused);}} className="px-4 py-2 bg-gray-700 hover:bg-gray-600 rounded-xl shadow text-white" >
            {paused ? "Reprendre" : "Pause"}
          </button>
<ChronoCercle timeLeft={timeLeft} duration={transitionTime} text={formatDureeTexte(timeLeft)} />
          <button onClick={() => {setStepIndex(prev => prev + 1);}} className="px-4 py-2 bg-orange-700 hover:bg-orange-600 rounded-xl shadow text-white" >
            Passer
          </button>
        </div>
      </div>
